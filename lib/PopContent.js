...
import { View, Text, TouchableOpacity,TouchableHighlight, Dimensions, Platform, PixelRatio } from 'react-native';
...
export default class PopContent extends Component {
	...
	render() {
		...
		Array.prototype.forEach.call(btns, (btn, index) => {
			btnContent.push(
				<TouchableHighlight style={buttonFlowColumn ? styleSheet.btnTextBoxMulti : styleSheet.btnTextBox} onPress={btn.callback} key={`btnTextBox${index}`}>
		...
		const stbtnBox = isTablet() ? styleSheet.btnBoxTab : styleSheet.btnBox;
		const stbtnBoxMulti = isTablet() ? styleSheet.btnBoxMultiTab : styleSheet.btnBoxMulti;
		...
		return (
			<View style={styleSheet.tipBox}>
				...
				<View style={styleSheet.line}></View>
				 <View style={buttonFlowColumn ? stbtnBoxMulti : stbtnBox}> 
					{btnContent}
				</View>
			</View>
		);
