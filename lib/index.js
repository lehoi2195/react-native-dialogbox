...
export default class DialogBox extends Component {
	...
	_renderContent = (styles) => {
		const stBoxView = isTablet() ? styles.tipBoxViewTab : styles.tipBoxView;
		return (
			<View style={stBoxView}>
				{this.state.content}
			</View>
		);
	}
	...
	render() {
		...
		const transform = this.props.rotate ? { transform: [{ rotate: '90deg' },] } : {};

		if (isVisible) {
			return (
				<View style={[styles.popupContainer, transform]}>
